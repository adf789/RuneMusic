...
namespace MidiPlayerTK
{
    public class TestMidiStream : MonoBehaviour
    {
        ...
        private void GUIForScale()
        {
            ...
            {
                RangeChord--;
                if (RangeChord < 0) RangeChord = MPTKRangeLib.RangeCount - 1;
            ...
            string strScale = GUILayout.TextField(RangeChord.ToString(), 2, GUILayout.Width(50));
            int scale = 0;
            try
            {
            ...
            if (scale != RangeChord)
            {
                RangeChord = Mathf.Clamp(scale, 0, MPTKRangeLib.RangeCount - 1);
                // Select the range from a list of range. See file Resources/GeneratorTemplate/GammeDefinition.csv 
                midiStreamPlayer.MPTK_RangeSelected = RangeChord;
                Play(false);
            }
            ...
            {
                RangeChord++;
                if (RangeChord >= MPTKRangeLib.RangeCount) RangeChord = 0;
                // Select the range from a list of range. See file Resources/GeneratorTemplate/GammeDefinition.csv 
                midiStreamPlayer.MPTK_RangeSelected = RangeChord;
                Play(false);
            }
        ...
        private void PlayRange()
        {
            ...
            MPTKRangeLib range = MPTKRangeLib.Range(RangeChord, true);
            for (int ecart = 0; ecart < range.Count; ecart++)
            {
                NotePlaying = new MPTKEvent()
                {
                    ...
                    Channel = StreamChannel,
                    Duration = DelayPlayScale, // millisecond, -1 to play undefinitely
